...
- **Flat-конфигурация ESLint 9**: основной пресет описан в `eslint.config.mjs` и использует типобезопасные настройки `typescript-eslint`.
- **Профили под разные задачи**:
  - базовый (`configs/base.mjs`) с Prettier и сортировкой импортов;
  - строгий (`configs/strict.mjs`) с ограничением `any`, соглашениями об именовании и правилом «один публичный тип на файл»;
  - React (`configs/react.mjs`) c правилами для JSX, хуков и доступности.
- **Типо-ориентированный анализ**: линтер запускается в режиме `recommendedTypeChecked`, поэтому в проекте должен существовать актуальный `tsconfig.json`.
- **Готовые исключения**: из проверки убираются артефакты сборки, конфиги, документация и JSON/YAML-файлы (`configs/rules/ignore.rule.mjs`), чтобы отчёт линтера фокусировался на исходниках.
